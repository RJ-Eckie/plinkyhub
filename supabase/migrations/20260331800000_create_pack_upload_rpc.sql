-- RPC function that inserts all pack-related DB rows in a single transaction.
-- Storage uploads happen on the client before calling this function.
-- If any insert fails the entire transaction is rolled back and a descriptive
-- error is raised.
...
BEGIN
  ...
  LOOP
    INSERT INTO samples (
      ...
      slice_points, base_note, fine_tune, pitched, slice_notes
    ) VALUES (
  ...
  IF wavetable_data IS NOT NULL THEN
    INSERT INTO wavetables (user_id, name, file_path, description, is_public)
    VALUES (
  ...
  LOOP
    INSERT INTO patterns (user_id, name, file_path, description, is_public)
    VALUES (
