-- Add content_hash column to packs for deduplication.
ALTER TABLE packs ADD COLUMN IF NOT EXISTS content_hash text;

CREATE INDEX IF NOT EXISTS idx_packs_content_hash
  ON packs (content_hash)
  WHERE content_hash IS NOT NULL;

-- Update the RPC to accept and store the pack content_hash.
...
BEGIN
  ...
  -- Insert the pack (now with content_hash).
  INSERT INTO packs (
